...
var highlightStyle = { 
    ...
    fillOpacity: 1,
    stroke: true,
    opacity: 1 // Borde totalmente visible al seleccionar
};
...
// --- 6. CARGA DE DATOS Y ESTADÍSTICAS ---
async function cargarDatos() {
    try {
        const responseZonificacion = await fetch('zonificacion.geojson'); 
        if (!responseZonificacion.ok) throw new Error("Falta 'zonificacion.geojson'");
        const zonificacion = await responseZonificacion.json();
        zonificacionData = zonificacion;

        renderMap(zonificacionData);
        ...
        zonificacionData.features.forEach(function(feature) {
